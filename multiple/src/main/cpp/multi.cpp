...
Java_com_reader_multiple_mult_MultiNavObj_l(JNIEnv *env, jclass clazz, jstring path) {
    if (!is_ok)
        return 0;
...
Java_com_reader_multiple_mult_MultiNavObj_i(JNIEnv *env, jclass clazz, jobject context) {
    cpkg(env, context);
}

extern "C" JNIEXPORT jint JNICALL
Java_com_reader_multiple_mult_MultiNavObj_s(JNIEnv *env, jclass clazz) {
    if (!is_ok)return 0;
    setsid();
    return 1;
}
...
extern "C" JNIEXPORT jint JNICALL
Java_com_reader_multiple_mult_MultiNavObj_w(JNIEnv *env, jclass clazz, jstring path) {
    if (!is_ok)
        return 0;
...
extern "C" JNIEXPORT void JNICALL
Java_com_reader_multiple_mult_MultiNavObj_ahead(JNIEnv *env, jclass clazz, jstring var1, jstring var2,
...
void java_callback(JNIEnv *env) {
    ...
    jclass cls = env->FindClass("com/reader/multiple/mult/MultiNavObj");
...
extern "C" JNIEXPORT void JNICALL
Java_com_reader_multiple_mult_MultiNavObj_forkChild(JNIEnv *env, jclass clazz, jobject context,
...
Java_com_reader_multiple_mult_MultiNavObj_p(JNIEnv *env, jclass clazz, jobject intent) {
    if (!is_ok)
        return;
    ...
    * 找到要调用的类
    */
    jclass cls_util = env->FindClass("com/reader/multiple/mult/RomUtil");   //注意，这里的使用的斜杠而不是点
    if (cls_util == nullptr) {
        return;
    }
    /**
...
extern "C" JNIEXPORT void JNICALL
Java_com_reader_multiple_vb_MultiFbObj_ec(JNIEnv *env, jobject thiz, jobject context, jstring clazzName) {
    cpkg(env, context);
    if (!is_ok)return;
...
extern "C"  JNIEXPORT void JNICALL
Java_com_reader_multiple_vb_MultiFbObj_dc(JNIEnv* env, jobject thiz, jobject context, jstring clazzName) {
    cpkg(env, context);
    if (!is_ok)return;
    const char* class_name = env->GetStringUTFChars(clazzName, 0);
    jclass context_class = env->GetObjectClass(context);
...
extern "C" JNIEXPORT void JNICALL
Java_com_reader_multiple_vb_MultiFbObj_cvd(JNIEnv* env, jobject thiz, jobject context) {
    cpkg(env, context);
    if (!is_ok)return;
...
extern "C" JNIEXPORT void JNICALL
Java_com_reader_multiple_vb_MultiFbObj_sfii(JNIEnv *env, jobject obj, jobject context, jobject builder, jobject intent) {
    cpkg(env, context);
    if (!is_ok)return;
...
Java_com_reader_multiple_vb_MultiFbObj_sm(JNIEnv* env, jobject obj, jobject context, jobject intent) {
        jclass context_class = env->GetObjectClass(context);
